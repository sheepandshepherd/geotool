...
__gshared GLuint g_programuColor = 0;
__gshared uint vVertexPosition;
__gshared uint vVertexTexCoord;
__gshared uint vVertexColor;
...
{
	///import main : texturedRectShader, texturedRectShaderIDs;
    for (int i = 0; i < CIRCLE_VERTS; ++i)
    {
	...
	// -------  pixelHeight was 15.0f
    const result = stbtt_BakeFontBitmap(ttfBuffer, 0, 18.0f, bmap, 
	...
	/// should these be replaced? non-varying indices ?= gl3
    glBindBuffer(GL_ARRAY_BUFFER, g_vbos[0]);
    ...
    string fs =
		...
		"    else{\n"
		"        vec4 tc = texture2D(Texture, texCoord).rgba;\n"
		"        \n"
    ...
    /++glBindAttribLocation(g_program, 0, "VertexPosition"); // replaced; locations are held in variables vVertex...
    glBindAttribLocation(g_program, 1, "VertexTexCoord");
    glBindAttribLocation(g_program, 2, "VertexColor");+/
    //glBindFragDataLocation(g_program, 0, "Color"); // gl_Color replaces this
	...
	/// ------------------------------------------------------------------------
	vVertexPosition = glGetAttribLocation(g_program,"VertexPosition");
	vVertexTexCoord = glGetAttribLocation(g_program,"VertexTexCoord");
	vVertexColor = glGetAttribLocation(g_program,"VertexColor");
	/// ------------------------------------------------------------------------
	if (!g_vao) glGenVertexArrays(1, &g_vao);
	glGenBuffers(3, g_vbos.ptr);
	...
	glEnableVertexAttribArray(vVertexTexCoord);
	glEnableVertexAttribArray(vVertexColor);
	
	/// should these be replaced? non-varying indices ?= gl3
	glBindBuffer(GL_ARRAY_BUFFER, g_vbos[0]);
	glVertexAttribPointer(vVertexPosition, 2, GL_FLOAT, GL_FALSE, GL_FLOAT.sizeof * 2, null);
	glBufferData(GL_ARRAY_BUFFER, 0, null, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, g_vbos[1]);
	glVertexAttribPointer(vVertexTexCoord, 2, GL_FLOAT, GL_FALSE, GL_FLOAT.sizeof * 2, null);
	glBufferData(GL_ARRAY_BUFFER, 0, null, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, g_vbos[2]);
	glVertexAttribPointer(vVertexColor, 4, GL_FLOAT, GL_FALSE, GL_FLOAT.sizeof * 4, null);
	glBufferData(GL_ARRAY_BUFFER, 0, null, GL_STATIC_DRAW);
	/// ------------------------------------------------------------------------
	...
	g_programuColor = glGetUniformLocation(g_program, "uColor");
...
/// TEXTURED RECT -- ADDED
void drawTexturedRect(float x, float y, float w, float h, float fth, uint col, uint texID)
{
	...
	glUniform4f(g_programuColor,cast(float)(col & 0xff) / 255.0, cast(float)((col >> 8) & 0xff) / 255.0, cast(float)((col >> 16) & 0xff) / 255.0, fmax(cast(float)((col >> 24) & 0xff) / 255.0, 0.11f));

	//(verts[0..1]~verts[2..3]~verts[4..5]~verts[4..5]~verts[6..7]~verts[0..1])
	//(uvs[0..1]~uvs[2..3]~uvs[4..5]~uvs[4..5]~uvs[6..7]~uvs[0..1])

	glBindVertexArray(g_vao);
	glBindBuffer(GL_ARRAY_BUFFER, g_vbos[0]);
	glBufferData(GL_ARRAY_BUFFER, 8 * float.sizeof, verts.ptr, GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, g_vbos[1]);
	glBufferData(GL_ARRAY_BUFFER, 8 * float.sizeof, uvs.ptr, GL_STATIC_DRAW);
	//glBindBuffer(GL_ARRAY_BUFFER, g_vbos[2]);
	//glBufferData(GL_ARRAY_BUFFER, cSize * float.sizeof, c, GL_STATIC_DRAW);
...
void imguiRenderGLDraw(int width, int height)
{
    ...
    glUseProgram(g_program);
    glActiveTexture(GL_TEXTURE1); /// was 0
    glUniform2f(g_programViewportLocation, cast(float)width, cast(float)height);
    glUniform1i(g_programTextureLocation, 1); /// was 0
	glUniform4f(g_programuColor, 0f, 0f, 0f, 0f);
    ...
    for (int i = 0; i < nq; ++i)
    {
		...
		else if (cmd.type == IMGUI_GFXCMD_TEXTUREDRECT)
		{
			/+texturedRectShader.bind();
			glActiveTexture(GL_TEXTURE1); /// was 0
			glUniform2f(texturedRectShaderIDs[3], cast(float)width, cast(float)height);
			glUniform1i(texturedRectShaderIDs[4], 1); /// was 0+/

			drawTexturedRect(cast(float)cmd.texturedRect.x * s + 0.5f, cast(float)cmd.texturedRect.y * s + 0.5f,
			                 cast(float)cmd.texturedRect.w * s - 1, cast(float)cmd.texturedRect.h * s - 1,
				         	 1.0f, cmd.col, cmd.texturedRect.texID);
